...
class MedicoListCreateView(generics.ListCreateAPIView):
    ...
    def post(self, request, *args, **kwargs):
        print("POST a Medicos")
        usuarioData = request.data.pop('7')
        serializerU  = UserSerializer(data=usuarioData)
        serializerU.is_valid(raise_exception=True)
        usuario = serializerU.save()
        enfData = request.data   
        enfData.update({"usuario":usuario.documento_medico})
...
class MedicoRetrieveUpdateView(generics.RetrieveUpdateAPIView):
    queryset = Medicos.objects.all()
    serializer_class = MedicoSerializer
    lookup_field = "documento_medico"             # campo con el que se realiza la búsqueda de los objetos
